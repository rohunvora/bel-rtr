  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-[68px] bg-[#191a1a] border-r border-[#2d2e2f] flex flex-col items-center py-4 z-40">
      ...
      <Link href="/" className="mb-6">
        <div className="w-10 h-10 rounded-lg bg-[#242526] flex items-center justify-center hover:bg-[#2d2e2f] transition-colors">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
      ...
      {/* New Trade Button - only show on dashboard */}
      {onNewTrade && (
        <button
          onClick={onNewTrade}
...
function NavItem({
  ...
  return (
    <button
      className="relative w-10 h-10 rounded-lg flex flex-col items-center justify-center transition-colors group hover:bg-[#242526] text-[#6b6c6d] hover:text-[#9a9b9c]"
      title={label}
    >
      {icon}
      {/* Tooltip */}
